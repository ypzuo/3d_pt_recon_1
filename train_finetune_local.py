...
def get_epoch_loss(val_models, val_pair_indices):
	...
	for b in xrange(batches):
		batch_ip, batch_gt, batch_img = fetch_batch_dense(val_models, val_pair_indices, b, BATCH_SIZE)
		runlist = [loss, chamfer_distance_scaled, dists_forward_scaled, dists_backward_scaled]
		L,C,F,B = sess.run(runlist, feed_dict={pcl_in:batch_ip, pcl_gt:batch_gt, img_inp: batch_img})
	...
	with tf.Session(config=config) as sess:
		...
		for i in xrange(start_epoch, max_epoch): 
			...
			for b in xrange(batches):
				...
				batch_ip, batch_gt, batch_img = fetch_batch_dense(train_models, train_pair_indices, b, BATCH_SIZE)

				runlist = [loss, chamfer_distance, dists_forward, dists_backward, optim]
				L, C, F, B, _ = sess.run(runlist, feed_dict={pcl_in:batch_ip, pcl_gt:batch_gt, img_inp: batch_img})
				_summary_losses = [L, C, F, B]
