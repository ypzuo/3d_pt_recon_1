...
def get_epoch_loss(val_models, val_pair_indices):
	...
	for b in xrange(batches):
		...
		runlist = [loss, L1, L2, chamfer_distance_rimg_scaled, dists_forward_rimg_scaled, dists_backward_rimg_scaled]
		_l, _l1, _l2, C, F, B = sess.run(runlist, feed_dict={pcl_gt:batch_gt, img_inp:batch_ip})
		_summary_losses = [F, B, C, _l1, _l2, _l]
...
if __name__ == '__main__':
	...
	with tf.variable_scope('psgn_vars'):
		z_latent_img = image_encoder_r(img_inp, FLAGS)
	...
	ph_summary = [ph_dists_forward, ph_dists_backward, ph_dists_chamfer, ph_l1, ph_l2, ph_loss]
	...
	with tf.Session(config=config) as sess:
		...
		for i in xrange(start_epoch, max_epoch):
			...
			for b in xrange(batches):
				...
				batch_ip, batch_gt = fetch_batch(train_models, train_pair_indices, b, BATCH_SIZE)
				runlist = [loss, L1, L2, chamfer_distance_rimg, dists_forward_rimg, dists_backward_rimg, optim]
				_l, _l1, _l2, C, F, B, _ = sess.run(runlist, feed_dict={pcl_gt:batch_gt, img_inp:batch_ip})
				_summary_losses = [F, B, C, _l1, _l2, _l]
