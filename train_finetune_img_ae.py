...
NUM_POINTS = 1024					# Number of points predicted
...
def get_epoch_loss(val_models, val_pair_indices):
	...
	for b in xrange(batches):
		batch_ip, batch_gt = fetch_batch(val_models, val_pair_indices, b, VAL_BATCH_SIZE)
		runlist = [loss, chamfer_distance_rimg_scaled, dists_forward_rimg_scaled, dists_backward_rimg_scaled]
		_l, C, F, B = sess.run(runlist, feed_dict={pcl_gt:batch_gt, img_inp:batch_ip})
		_summary_losses = [F, B, C, _l]
...
if __name__ == '__main__':
	...
	saver_ae = tf.train.Saver(var_list=pointnet_ae_vars)
	saver_psgn =  tf.train.Saver(var_list=psgn_vars)
	...
	ph_summary = [ph_dists_forward, ph_dists_backward, ph_dists_chamfer, ph_loss]
	...
	with tf.Session(config=config) as sess:
		...
		for i in xrange(start_epoch, max_epoch):
			...
			for b in xrange(batches):
				...
				batch_ip, batch_gt = fetch_batch(train_models, train_pair_indices, b, BATCH_SIZE)
				runlist = [loss, chamfer_distance_rimg, dists_forward_rimg, dists_backward_rimg, optim]
				_l, C, F, B, _ = sess.run(runlist, feed_dict={pcl_gt:batch_gt, img_inp:batch_ip})
				_summary_losses = [F, B, C, _l]
