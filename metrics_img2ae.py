...
if __name__ == '__main__':
	...
	with tf.variable_scope('pointnet_ae') as scope:
		z_latent_pcl = encoder_with_convs_and_symmetry(in_signal=reconstr_img, n_filters=[64,128,128,256,bneck_size], 
			filter_sizes=[1],
			strides=[1],
			b_norm=True,
			verbose=True,
	...
	if FLAGS.load_best:
		...
		snapshot_folderae = join(FLAGS.ae_logs, 'best')
	else:
